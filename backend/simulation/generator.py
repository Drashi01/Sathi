...
from .models import IncidentState, VehicleState, QuadrantState, TrafficZone
...
def generate_traffic_zones() -> List[TrafficZone]:
    return [
        ...
        TrafficZone(
            id="TZ-2",
            name="North Expressway Bottleneck",
            x_min=15.0,
            x_max=45.0,
            y_min=70.0,
            y_max=85.0,
        ...
        TrafficZone(
            id="TZ-3",
            name="South-East Industrial Delay",
            x_min=65.0,
            x_max=90.0,
            y_min=15.0,
            y_max=35.0,
    ...
    for i in range(count):
        ...
        incidents.append(
            IncidentState(
                ...
                arrival_time=arr,
                priority=p,
                priority_weight=PRIORITY_WEIGHTS[p],
                service_time=8,
                status="unassigned"
            )
        )
...
def generate_initial_vehicles() -> List[VehicleState]:
    ...
    for quad_id, coords in quad_offsets.items():
        for x, y in coords:
            v_id = f"V-{v_counter:02d}"
            name = f"Ambulance {v_id}"
            vehicles.append(
                VehicleState(
                    id=v_id,
                    name=name,
                    home_quadrant=quad_id,

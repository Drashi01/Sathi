...
import { Eye, Activity, AlertTriangle, TrafficCone, Compass, Camera, Truck, Users, Clock, Flame, Siren, Droplets } from 'lucide-react';
...
export default function LiveGrid({ snapshot, currentStep, cinematicMode, toggleCinematicMode }) {
  ...
  useEffect(() => {
    ...
    // 🎥 Cinematic Camera Auto-Focus Target
    let targetX = width / 2;
    let targetY = height / 2;
    let targetZoom = 1.0;

    if (cinematicMode) {
      const p3Inc = snapshot.incidents.find(i => (i.priority === 3 || i.fire_severity === 3 || i.threat_level === 3) && i.status !== 'completed');
      const busyVeh = snapshot.vehicles.find(v => v.status === 'moving' || v.status === 'busy' || v.status === 'rebalancing' || v.status === 'refilling');
    ...
    // 3. Hydrant Stations (Fire & Unified Scenarios)
    if (snapshot.hydrants && (snapshot.scenario === "fire" || snapshot.scenario === "unified")) {
      snapshot.hydrants.forEach(h => {
        const hx = toCanvasX(h.x);
        const hy = toCanvasY(h.y);
    ...
    // 4. Grid Lines & Quadrant Divisions
    ...
    // Quadrant Badges
    const quads = [
      { id: 1, label: 'Q1 (North-West)' },
      { id: 2, label: 'Q2 (North-East)' },
      { id: 3, label: 'Q3 (South-West)' },
      { id: 4, label: 'Q4 (South-East)' },
    ];
    ...
    snapshot.vehicles.forEach(v => {
      if (v.target_x !== null && v.target_y !== null) {
        ...
        if (v.status === 'refilling') {
          ctx.strokeStyle = '#0284C7'; // Hydrant Refill Blue
          ctx.lineWidth = 1.5;
        } else if (v.status === 'rebalancing') {
          ctx.strokeStyle = '#A855F7'; // Purple Rebalancing
          ctx.lineWidth = 1.5;
        } else {
          const isP3 = v.patients.some(p => p.priority === 3);
          ctx.strokeStyle = v.agency === "fire" ? '#F59E0B' : v.agency === "police" ? '#EF4444' : isP3 ? '#EF4444' : '#00F0FF';
    ...
    // 6. Draw Incidents (Domain specific icons)
    snapshot.incidents.forEach(inc => {
      ...
      let color = '#3B82F6';
      let label = `P${inc.priority}`;
      let radius = 6;

      if (inc.agency === "fire") {
        color = inc.priority === 3 ? '#EF4444' : inc.priority === 2 ? '#F59E0B' : '#EAB308';
        label = `F${inc.priority}`;
      } else if (inc.agency === "police") {
        color = inc.priority === 3 ? '#EF4444' : inc.priority === 2 ? '#F59E0B' : '#3B82F6';
        label = `T${inc.priority}`;
      }
    ...
    // 7. Draw Vehicles (Domain specific styling)
    snapshot.vehicles.forEach(v => {
      const vx = toCanvasX(v.x);
      const vy = toCanvasY(v.y);
      ...
      if (v.status === 'refilling') {
        ...
        color = '#A855F7';
      } else if (v.occupied_slots >= v.capacity || v.water_level < 20.0) {
        color = '#EF4444';
      } else if (v.occupied_slots > 0 || v.status === 'moving') {
        color = '#F59E0B';
      }
      ...
      let badgeText = `[${v.occupied_slots}/${v.capacity}]`;
      if (v.agency === "fire") {
        badgeText = `${v.water_level.toFixed(0)}%`;
      } else if (v.status === 'rebalancing') {
        badgeText = '[REBAL]';
      }
      ...
      ctx.fillStyle = color;
      ctx.font = 'bold 8px monospace';
      ctx.fillText(badgeText, vx, vy + 22);
    });
  ...
  const handleMouseMove = (e) => {
    ...
    snapshot.vehicles.forEach(v => {
      ...
      if (Math.hypot(mouseX - vx, mouseY - vy) <= 16) {
        found = v;
      }
    });
  ...
  return (
    <div className="bg-[#131B2E] border border-slate-800 rounded-2xl p-4 shadow-2xl relative flex flex-col items-center">
      {/* Header Bar */}
      <div className="w-full flex items-center justify-between mb-3 px-2 font-mono">
        <div className="flex items-center gap-2">
          <Activity className="w-5 h-5 text-cyan-400" />
          <h2 className="text-base font-bold text-slate-100 uppercase">
            100x100 {snapshot?.scenario?.toUpperCase() || 'LIVE'} DISPATCH GRID
          </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Camera className="w-3.5 h-3.5" /> Cinematic {cinematicMode ? 'ON' : 'OFF'}
          </button>
      ...
      <div className="relative border-2 border-slate-800 rounded-xl overflow-hidden shadow-inner bg-[#090D16]">
        ...
        {cinematicMode && snapshot?.recent_decisions?.length > 0 && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md border border-purple-500 text-purple-300 px-4 py-1.5 rounded-full text-xs font-mono font-bold flex items-center gap-2 shadow-2xl animate-bounce">
            ...
            Dispatching {snapshot.recent_decisions[0].vehicle_name} to {snapshot.recent_decisions[0].incident_id}
          </div>
        )}
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-800 pb-1.5 mb-2">
              <span className="font-bold text-cyan-300 flex items-center gap-1">
                {hoveredVehicle.agency === "fire" ? <Flame className="w-3.5 h-3.5 text-amber-400" /> : <Truck className="w-3.5 h-3.5 text-cyan-400" />} {hoveredVehicle.name}
              </span>
              <span className="text-[10px] text-slate-400">Q{hoveredVehicle.current_quadrant}</span>
            </div>

            {hoveredVehicle.agency === "fire" ? (
              <div className="text-[11px] mb-2 flex items-center justify-between">
                <span>Water Level:</span>
                <span className="font-bold text-amber-400">{hoveredVehicle.water_level.toFixed(0)}%</span>
              </div>
            ) : (
              <div className="text-[11px] mb-2 flex items-center justify-between">
                <span>Slots Occupied:</span>
                <span className="font-bold text-cyan-400">[{hoveredVehicle.occupied_slots}/{hoveredVehicle.capacity}]</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="w-full mt-4 flex flex-wrap items-center justify-between gap-4 text-xs font-mono bg-[#0B0F19] p-3 rounded-xl border border-slate-800/80">
        <div className="flex items-center gap-3">
          ...
          <span className="flex items-center gap-1 text-emerald-400">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-400"></span> Idle
          </span>
          <span className="flex items-center gap-1 text-amber-400">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400"></span> Active
          </span>
